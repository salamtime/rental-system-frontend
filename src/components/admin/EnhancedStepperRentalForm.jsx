...
const EnhancedStepperRentalForm = ({ onSubmit, initialData = null, isLoading = false, onCancel }) => {
  ...
  // NEW: Step transition cooldown to prevent auto-submission
  const [stepTransitionCooldown, setStepTransitionCooldown] = useState(false);
  ...
  // NEW: Cleanup step transition timer on unmount
  useEffect(() => {
    return () => {
    ...
    try {
      ...
      if (customerId) {
        ...
        if (fetchResult.success && fetchResult.data) {
          ...
          console.warn('⚠️ Could not fetch complete customer data, using OCR data');
        }
      }
      ...
      console.error('❌ Error handling ID scan:', error);
  ...
  // NEW: Enhanced handleNext with cooldown activation
  const handleNext = () => {
    if (validateStep(currentStep)) {
      console.log('🔄 Step transition initiated from Step', currentStep);
      ...
      // Activate cooldown to prevent immediate submission
      setStepTransitionCooldown(true);
      console.log('🛡️ COOLDOWN: Activated 300ms cooldown to prevent auto-submission');
      
      // Set timer to deactivate cooldown after 300ms
      stepTransitionTimerRef.current = setTimeout(() => {
        setStepTransitionCooldown(false);
        console.log('✅ COOLDOWN: Deactivated - form submission now allowed');
      }, 300);
  ...
  // Prevent form submission on Enter key when not on Step 3
  const handleFormKeyDown = (e) => {
    if (e.key === 'Enter' && currentStep !== 3) {
      e.preventDefault();
      console.log('🛡️ GATEKEEPER: Prevented Enter key submission on Step', currentStep);
      return false;
    }
  };

  // === FORM SUBMISSION with ENHANCED GATEKEEPER LOGIC ===
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    // CRITICAL: Prevent submission if not on final step
    if (currentStep !== 3) {
      console.warn('⚠️ GATEKEEPER: Blocked submission attempt on Step', currentStep);
      return;
    }
    
    // NEW: Check cooldown status
    if (stepTransitionCooldown) {
      console.warn('🛡️ COOLDOWN GATEKEEPER: Blocked submission during step transition cooldown');
      return;
    }
    ...
    try {
      console.log('🚀 [StepperForm] Starting submission...');
      ...
      console.log('🧹 Cleaned submissionReadyFormData, removed: status, linked_display_id, booking_range, vehicle');
      ...
      if (isStaff && isPriceOverride) {
        ...
        console.log('🛡️ GATEKEEPER: Approval data set:', {
        ...
        try {
          ...
          if (notificationCount > 0) {
            ...
            console.log('📱 WHATSAPP: No admins were notified');
        ...
        console.log('🛡️ GATEKEEPER: Admin/Owner bypass - setting auto approval status');
        submissionData.approval_status = 'auto';
        submissionData.pending_total_request = null;
      } else {
        console.log('🛡️ GATEKEEPER: No price override or non-staff user - setting auto approval status');
        submissionData.approval_status = 'auto';
        submissionData.pending_total_request = null;
      }
      ...
      if (result && result.success) {
        let successMessage = '✅ Rental successfully created!';
      ...
      if (err.message && (err.message.includes('Vehicle availability check failed') || 
        ...
        setErrors({ 
          general: `🚫 Vehicle Scheduling Conflict: ${err.message}`,
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      ...
      {/* Form Content */}
      <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
              ...
              <div>
                ...
                {formData.rental_type === 'hourly' && (
                  <p className="text-xs text-purple-600 mt-2">
                    ⏰ Time fields auto-populated with Morocco local time + 30 minutes
                  </p>
                )}
              </div>
        ...
        {/* Step 3: Review & Payment - KEEPING ORIGINAL CONTENT */}
        {currentStep === 3 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600 mt-1">💳 Auto-updates based on deposit amount</p>
                </div>
