...
class GeminiVisionOCR {
  constructor() {
    ...
    // Updated to use gemini-2.5-flash (most likely to work with current API)
    this.apiUrl = `https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key=${this.apiKey}`;
    this.listModelsUrl = `https://generativelanguage.googleapis.com/v1/models?key=${this.apiKey}`;
  }
  ...
  async listAvailableModels() {
    try {
      ...
      const visionModels = result.models?.filter(model => 
        ...
        (model.name.includes('gemini') || model.name.includes('vision'))
      ) || [];
  ...
  async processIdDocument(imageFile, customerId = null) {
    try {
      ...
      // Save extracted data to database using UnifiedCustomerService
      if (customerId) {
        ...
        if (!saveResult.success) {
          ...
          // Fetch the saved customer data to verify what was actually stored
